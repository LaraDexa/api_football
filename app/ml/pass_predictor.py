...
# Cache global de pipelines por jornada
_pipeline_cache = {}
...
def _train_global(jornada: int):
    """Entrena con TODOS los datos hasta la jornada indicada."""
...
def _train_player(df_player: pd.DataFrame):
    """Entrena solo con los datos del jugador."""
    n = len(df_player)
...
def predict_player(player_id: int, jornada: int) -> dict:
    ...
    # 1) Datos del jugador hasta jornada-1
    df_player = df[(df["Id"] == player_id) & (df["jornada"] <= jornada - 1)]
    if len(df_player) >= 5:
        pipelines = _train_player(df_player)
        df_for_test = df
    else:
        pipelines, df_for_test = _train_global(jornada - 1)

    # 2) Elegir fila de test
...
def append_stats_and_retrain(new_row: dict, jornada: int) -> bool:
    ...
    df1 = df0.append(new_row, ignore_index=True)
